...
class Topology(Topo):
    def build(self):
        ...
        r110, r120, r130, r210, r310, r410 = [
            ...
            for n, l in nodes_with_loopbacks.items()
        ]

        hosts = {
            "h211": {"ip": "10.2.1.1/24", "droute": "via 10.2.1.254"},
            "h311": {"ip": "10.3.1.1/24", "droute": "via 10.3.1.254"},
            "h411": {"ip": "10.4.1.1/25", "droute": "via 10.4.1.126"},
            "h412": {"ip": "10.4.1.129/25", "droute": "via 10.4.1.254"},
        }

        # Hosts
        h211, h311, h411, h412 = [
            self.addHost(name=h, ip=details["ip"], defaultRoute=details["droute"])
            for h, details in hosts.items()
        ]
